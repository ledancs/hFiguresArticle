...
The hFigures library provides features to improve the understanding of health data. The library enables comparison of multiple health measurements and improves the understanding of the interrelationships between the data sources. It also organizes the measurements in groups separated by a structured layout. The purpose is to enable a visual representation of the evolution of health measurements in a comprehensive and holistic manner. Other additional improvements include exporting the visualization as an SVG file, informative color-coded figures, label space distribution and other design improvements that enhance readability while reducing clutter.

\parttitle{Conclusions} %if any
We developed hFigures, an open source library for visualizing a holistic, normalized and comprehensive graphical representation of health data. The idea is based on the concept of the hGraph but it provides additional key features, including a comparison of multiple health measurements over time.
\end{abstract}
...
%The users participating in the design of the library had several areas of expertise, from Signal Processing to Machine Learning, Statistics, Software Engineering and User Experience.
...
In order to test the hFigures library, we designed an application for health monitoring and evolution. The application visualized a health coaching program of a modeled patient. The application has thee components: activity timeline, the hFigures visualization and longitudinal measurements.

The \textbf{activity timeline} represents the health interventions the modeled patient has done over a period of time. At certain times, the modeled patient has a snapshot of a set of measruements visualized as \textbf{longitudinal measurements} also represented in the \textbf{hFigures} visualization. The changes in the status of the patient are displayed by two figures. Adjustable points in time animate two hFgures. The adjustable points are represented as lines in the activity timeline and the longitudinal measurements.
...
Usability testing requires at least one expert in the area of human-computer interaction. For our evaluation we recruited 3 experts who assess the application using Nielsen's heuristics \cite{nielsen1994usability}. The evaluation has 11 metrics evaluated using a seven point Likert scale. Experts assess metrics regarding the dialogue, memory load to use the system, error messages and prevention among other aspects.

\paragraph*{Controlled User Testing}
Rubin and Chisnell \cite{rubin2008handbook} as well as Nielsen \cite{nielsen1994usability} recommend to design a set of tasks for users to perform in a controlled environment. We can then measure the time to complete the tasks, errors (crucial and non-crucial) and whether the task was successfully completed.
...
\item \textbf{Project home page and source code repository:} \url{https://gitlab.com/ledancs/hFigures}
...
AL developed the implementation of the hFigures library using a user-centered approach. AL and HN wrote jointly the article. HN suggested several use cases that helped shaped the design process. HN contributed in the design and conception of the library.
