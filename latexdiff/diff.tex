...
%DIF DEL old.tex   Thu Jan 21 11:05:50 2016
%DIF ADD new.tex   Thu Jan 21 11:05:50 2016
%%                                      %
...
%DIFDELCMD <    %noteref={n1},
%DIFDELCMD <    email={hannu.nieminen.o@tut.fi}
%DIFDELCMD < ]{\inits{JRS}\fnm{Hannu} \snm{Nieminen}}
%DIFDELCMD < %%%
\DIFdelend \author[
   addressref={aff1},
   %noteref={n1},
...
The way we look at data has a great impact on how we can understand it, particularly when the data is related to health and wellness. Due to the increased use of self-tracking devices and the ongoing shift towards preventive medicine, better understanding of our health data is an important part of improving the general welfare of the citizens. Electronic Health Records, self-tracking devices and mobile applications provide a rich variety of data but it often becomes \DIFdelbegin \DIFdel{complex and }\DIFdelend difficult to understand. We implemented the hFigures library inspired on the hGraph visualization with additional improvements. The purpose of the library is to provide a visual representation of the evolution of health measurements in a \DIFdelbegin \DIFdel{comprehensive and holistic }\DIFdelend \DIFaddbegin \DIFadd{complete and useful }\DIFaddend manner. 
...
We developed hFigures, an open source library for visualizing a \DIFdelbegin \DIFdel{holistic, normalized and comprehensive }\DIFdelend \DIFaddbegin \DIFadd{complete, accurate and normalized }\DIFaddend graphical representation of health data. The idea is based on the concept of the hGraph but it provides additional key features, including a comparison of multiple health measurements over time. We conducted a \DIFdelbegin \DIFdel{comprehensive }\DIFdelend usability evaluation of the library as a key component of an application for health and wellness monitoring. The results indicate that the data visualization library was helpful in assisting users in understanding health data and its evolution over time.
...
To better understand our health, we need to combine heterogeneous data sources and present the information to the user \DIFdelbegin \DIFdel{with a comprehensive and holistic approach}\DIFdelend \DIFaddbegin \DIFadd{in a complete and accurate manner}\DIFaddend . In order to accomplish this, health information technologies and visualization design need to be integrated \cite{lesselroth2011data}. The goal is to provide tools for individuals to take better  decisions regarding their health. Similarly, doctors and other medical experts need tools and solutions for getting a \DIFdelbegin \DIFdel{comprehensive }\DIFdelend \DIFaddbegin \DIFadd{complete and accurate }\DIFaddend view of the patients health, combining together patient's own measurements and clinical data.
...
% Hannu's comment to address... (Hannu: I think this is now addressed after my changes, I now remove this comment .... removed :-)
...
Goetze \cite{goetz2015} demonstrated the impact of data visualization as means to represent health data in a \DIFdelbegin \DIFdel{comprehensive }\DIFdelend \DIFaddbegin \DIFadd{complete and accurate }\DIFaddend manner. He conducted a project that redesigned laboratory test results from numerical tables into colored graphics. He demonstrated that the patients were able to understand better their health situation when presented with the new designs.
...
As stated by Shneiderman \textit{et al.} \cite{shneiderman2013improving}, new visual representations are needed for ``systematic yet flexible visual analytics processes''. We present an existing tentative solution known as the hGraph, released publicly by MITRE corporation. We describe its main advantages and how they can address these challenges. In a previous study \cite{EMBCLedNiem} we identified possible improvements and based on our own implementation we addressed these issues and extended the features of the hGraph to better address \DIFdelbegin \DIFdel{complex }\DIFdelend \DIFaddbegin \DIFadd{large }\DIFaddend data sources.
...
The graphical representation of health data requires a \DIFdelbegin \DIFdel{holistic approach. A holistic }\DIFdelend \DIFaddbegin \DIFadd{complete and accurate }\DIFaddend overview of a patient \DIFdelbegin \DIFdel{usually is comprised of }\DIFdelend \DIFaddbegin \DIFadd{often including }\DIFaddend large amounts of measurements\DIFaddbegin \DIFadd{, }\DIFaddend which in turn translates to large datasets. Therefore, health data visualizations need to scale to accommodate large datasets. 
...
% Plotted values remain uniformly distributed along the circumference.
...
% Addressing Hannu's comment. We should refer to the EMBC article as an independent research not as "our" previous study.
% Cite the paper after the words .."... a previous study...."
...
In this chapter we \DIFdelbegin \DIFdel{detailed }\DIFdelend \DIFaddbegin \DIFadd{detail }\DIFaddend the implementation methodology and the key features implemented in the library. We named the library Health Figures (hFigures) because it is based on the design principles of the hGraph. hFigures makes an emphasis on multiple graphs, or figures, in order to provide a graphical representation of evolution of the data over time (multiple snapshots of the data at certain points in time).
...
\paragraph*{hFigures} The developed hFigures library is utilized to display the set of measurements taken during the health coaching program. These measurements describe \DIFdelbegin \DIFdel{a holistic view }\DIFdelend \DIFaddbegin \DIFadd{an overview }\DIFaddend of the health situation of the modeled patient. Users can change the time at which the snapshot was taken to compare changes over time as a result of the health interventions.
...
\DIFdelbegin \DIFdel{All participants signed an informed consent explaining the purpose of the tests and how the results were going to be used}\DIFdelend \DIFaddbegin 

\DIFadd{The experimental procedures described in this paper complied with the principles of Helsinki Declaration of 1975, as revised in 2000. All subjects gave informed consent to participate and they had a right to withdraw from the study at any time}\DIFaddend . The informed consent also explained that their names and identities will be kept confidential, that the results will not be linked to their identities\DIFdelbegin \DIFdel{and that }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend the sessions will be recorded using a Web camera and microphone for further study \DIFaddbegin \DIFadd{and that the clinical data visualized did not belong to a real person. A separate ethical committee approval was not required since this study was a usability testing process portraying simulated data not belonging to a real person. The results of the usability tests were kept anonymous and the collected data does not include sensitive information from the participants}\DIFaddend .
\DIFdelbegin \DIFdel{The participants had the right to withdraw from the tests at any time.
}\DIFdelend 
...
Other device manufacturers such as Withings or Jawbone provide their users the possibility to use their APIs to extract collected data in a similar way. As in the case of Fitbit, all data has at least a timestamp (date, time or both) and a set of values. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In order to specify the recommended range for these measurements in the hFigures data source file, it is possible to use websites such as WebMD or Mayo Clinic to obtain their recommendation for such values}\DIFdelend \DIFaddbegin \DIFadd{Recommended values for the health measurements can be obtained from healthcare professionals and public health information sources}\DIFaddend .

%DIF >  In order to specify the recommended range for these measurements in the hFigures data source file, it is possible to use websites such as WebMD or Mayo Clinic to obtain their recommendation for such values.
\DIFaddbegin 
...
\DIFdelbegin \DIFdel{Comprehensive and holistic }\DIFdelend \DIFaddbegin \DIFadd{Complete and accurate }\DIFaddend visualizations of health data \DIFaddbegin \DIFadd{have been thought to }\DIFaddend empower individuals, citizens and health professionals alike, to better understand situations and take better informed decisions \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{lesselroth2011data, shneiderman2013improving, rind2011interactive}}%DIFAUXCMD
}\DIFaddend . These decisions can be medical treatment, behaviour change practices, wellness development, health coaching program and more. In this article we detailed the underlying motivation to develop a visualization library inspired by the hGraph. 
...
\item \DIFaddbegin \textbf{\DIFadd{SciCrunch Resource ID}} \DIFadd{SCR\_014201
}\item \DIFaddend \textbf{Operating System:} Platform independent.
...
\DIFaddbegin \section*{\DIFadd{Availability of Data}}
\DIFadd{The dataset supporting the conclusions of this article is available in the BioSharing repository with the identifier biodbcore-000734 at the following url: }\url{https://biosharing.org/biodbcore-000734}\DIFadd{.
}

\DIFadd{The dataset is also available at Tampere University of Technology Personal Health Informatics website in the following url: }\url{http://www.tut.fi/phi/?p=319}\DIFadd{.
}
...
AL developed \DIFdelbegin \DIFdel{teh }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend hFigures library. AL and HN wrote jointly \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend article. HN suggested several use cases that helped shaped the design process. HN contributed in the design and conception of the library. AL, HN and MAM designed the application for health coaching. AL and MAM implemented the application. AL and MAM integrated the application into a Web service. MAM conducted the user testing and \DIFdelbegin \DIFdel{analyzed }\DIFdelend \DIFaddbegin \DIFadd{analysed }\DIFaddend the results. \DIFaddbegin \DIFadd{HN and AL wrote jointly the introduction and background. AL wrote the implementation and discussion. MAM and AL wrote jointly the evaluation and results section. HN and AL wrote jointly the conclusion.
    }\DIFaddend 

\section*{Acknowledgements}
This research was supported jointly by TEKES (the Finnish Funding Agency for Technology and Innovation) as part of the Digital Health Revolution \DIFaddbegin \DIFadd{project}\DIFaddend , as well as the European Commission and TEKES under the ARTEMIS-JU WithMe project.
...
% hFigures complex
\begin{figure}[h!]
  \caption{\DIFdelbeginFL %DIFDELCMD < \csentence{A complex hFigures example.} %%%
\DIFdelFL{A holistic health assessment }\DIFdelendFL \DIFaddbeginFL \csentence{A heterogeneous hFigures example.} \DIFaddFL{An overview }\DIFaddendFL of a modeled person comprised of several measurements with two time snapshots showing \DIFdelbeginFL \DIFdelFL{the person's }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{its }\DIFaddendFL evolution \DIFaddbeginFL \DIFaddFL{over time}\DIFaddendFL .}
...
% Data source
\begin{figure}[h!]
  \caption{\csentence{Health Monitoring Application.} The Health monitoring application with the three components: hFigures, Activity Timeline and Longitudinal Measurements.}
